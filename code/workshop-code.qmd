...
drought_exp_clean <- drought_exp |> 
  # lowercase and underscores in column names
  clean_names() |> 
  # creating new column with full scientific names
  # species codes to species names
  ...
  # creating new column with full treatment names 
  # water codes to water treatment
  mutate(water_treatment = recode_values(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  )) |>   
  # reordering levels of categorical variable (species name)
  ...
  mutate(water_treatment = as_factor(water_treatment),
         water_treatment = fct_relevel(water_treatment,
                                       "Drought stressed",
                                       "Well watered")) |>  
  # selecting specific columns for the data frame
...
slice_sample(
  drought_exp_clean, # using clean data fram
  n = 10 # show 10 row
)
...
ggplot(data = drought_exp_clean,
       mapping = aes(
       x = sla, # x-axis: specific leaf area
       y = total_g, # y-axis: total plant mass
       )) +
  # scatter plot
  geom_point(
    color = "blue",
  ...
  labs(
    ...
    x = expression("Specific leaf area (cm"^2~"/g)"),
    y = "Total mass (g)"
  )
...
## Model diagnostics (display these after selection)
...
- Residuals are homoscedastic (Residual v. Fitted/Scale Location),
...
- `species_namePenstemon centranthifolius`: difference between *Penstemon mass* in the *well watered* treatment and *Lotus scoparius mass* in the *well watered* treatment.

- `species_nameEschscholzia californica`: difference between *Eschsholzia californica mass* in the *well watered* treatment and *Lotus scoparius mass* in the *well watered* treatment.
...
preds <- ggpredict( 
  model4, # model object 
  terms = c("water_treatment", "species_name") # name predictors to create a df
) |> 
  # renaming columns to fit original df
...
ggplot(data = drought_exp_clean,
  ...
  # jitter: individual plants for each treatment in each species
  geom_jitter(height = 0,
              width = 0.1,
              alpha = 0.4,
              shape = 21) + 
  # pointrange: model prediction with 95% CI
  geom_pointrange(data = preds,
                  mapping = aes(x = water_treatment,
                                y = predicted,
                                ymin = conf.low,
                                ymax = conf.high)) +
  # create different panels for each species
  facet_wrap(~ species_name) +
  # remove legend
  theme(legend.position = "none") +
  # relabels x- and y- axes
  labs(x = "Treatment",
       y = "Total plant mass (g)")
